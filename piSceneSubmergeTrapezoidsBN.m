function [ submerged]  = piSceneSubmergeTrapezoidsBN(thisR, medium, varargin)
% Add participating media AND trapezoidal-pixels to a recipe
%
% Synopsis
...
%% Creation of Random Trapezoids

% Side lengths of trapezoids
...
for i = 1:length(loc) % Loop through Poisson-Disc Locations
    ...
    waterSideMesh.point3p = P(:); % Trapezoid vertices
    ...
    water.translation = {[xp; yp; inputs.sizeZ/2 + dz]};% Coordinates for trapezoidal-pixel
    ...
    % Add the Trapezoid-pixel to the Scene
    piAssetAdd(submerged, waterID, waterSide); % Place the modified Trapezoid Side/Plane
    submerged.set('medium', 'add', medium);     % Add seawater to the final recipie
