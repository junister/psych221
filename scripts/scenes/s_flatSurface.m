...
%%
thisR = piRecipeDefault('scene name','flatSurface','write', true);
...
ip = ipSet(ip, 'internalcolorspace', 'sensor');
ip = ipCompute(ip, sensor);
% ipWindow(ip);

%% Choose the rect
[locs,thisRectCenter] = vcROISelect(ip);
ip = ipSet(ip,'roi',thisRectCenter);
ipPlot(ip,'roi');

%%
roiRGBCenter = ipGet(ip,'roi data',thisRectCenter);
roirgCenter = chromaticity(roiRGB);
ieNewGraphWin;
plot(roirgCenter(:,1),roirgCenter(:,2),'b.');
...
%% Choose another rect
[locs,thisRectEdge] = vcROISelect(ip);
ip = ipSet(ip,'roi',thisRectEdge);
ipPlot(ip,'roi');

%%
roiRGBEdge = ipGet(ip,'roi data',thisRectEdge);
roirgEdge = chromaticity(roiRGB);
ieNewGraphWin;
