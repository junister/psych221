...
% To calculate the "film diagonal", we use the retina semi-diameter. The
% film diagonal is the diagonal of the rectangular image rendered out by
% PBRT, in real units. Since we restrict samples to a circular image, we
% can calculate the film diagonal to be the same as a square that
% circumscribes the circular image.
...
if(retinaRadius ~= 0)
    ...
    %
    %
    %     The distance between the zero point on the z-axis (i.e. the lens
    %     element closest to the sensor) and the dotted line will be equal
    %     to the "retinaDistance." The retina curvature is defined by the
    %     "retinaRadius" and it's height in the y and x direction is
    %     defined by the "retinaSemiDiam."
    %
    %
    %                                         :
    %                                         |  :
